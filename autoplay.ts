...
interface LegendMod {
  ...
  mapMinY: number;
  mapMaxY: number;
  leaderboard: Cell[]; // Assuming leaderboard is array of cells/objects
  sendPosition: (target: Cell, target2: Point) => void;
...
interface LegendModSettings {
  ...
  autoHideMass: boolean;
  hideMyName: boolean;
  hideTeammatesNames: boolean;
  showMass: boolean;
  hideEnemiesMass: boolean;
  autoHideFood: boolean;
  autoHideFoodOnZoom: boolean;
}

// Extend the global Window interface
declare global {
  interface Window {
    autoteammatenicks: Record<string, boolean> | any[];
    targetFood: Cell | null;
    autoPlay: boolean;
    doSplitFlag: boolean;
    VirusFlag: boolean;
    BiggerCellFlag: boolean;
    SmallerCellFlag: boolean;
    bestDist: number;
    teammatenicks: string[];

    // Distance Arrays
    DistanceX: Record<number, number>;
    DistanceY: Record<number, number>;
    DistanceName: Record<number, string>;
    DistanceId: Record<number, number>;

    DangerDistanceX: Record<number, number>;
    DangerDistanceY: Record<number, number>;
    DangerDistanceName: Record<number, string>;

    VirusDistanceX: Record<number, number>;
    VirusDistanceY: Record<number, number>;

    FlagDangerCells: number[];
    FlagVirusCells: number[];

    BadCellsDistanceX: Record<number, number>;
    BadCellsDistanceY: Record<number, number>;
    BadCellsDistanceName: Record<number, string>;

    SandwichCellCase: number | null;

    // External Libraries
    legendmod: LegendMod;
    legendmod5: LegendModSettings;
    application: { lastSentClanTag: string };
    $: any; // jQuery
  }
}
...
function calcTarget(): void {
  ...
  let target2: Point = { x: 0, y: 0 }; // Initialize to prevent undefined errors
  ...
  // Identify Biggest and Smallest owned cells
  let biggercell: Partial<Cell> = { mass: 0 };
  let smallercell: Partial<Cell> = { mass: 25000 };
  ...
  if (window.legendmod.playerCells.length > 0) {
    for (let i = 0; i < window.legendmod.playerCells.length; i++) {
      ...
      if (window.legendmod.playerCells[i].mass < (smallercell.mass || 25000)) {
  ...
  Object.keys(window.legendmod.cells).forEach((node) => {
    ...
    if (PlayerCell.nick != window.legendmod.playerNick) {
      try {
        window.DistanceX[PlayerCell.id] = PlayerCell.x -
          window.legendmod.playerX;
        window.DistanceY[PlayerCell.id] = PlayerCell.y -
          window.legendmod.playerY;
        ...
        // Logic: Close to a Virus
        if (
          distancePlayerCell <
            window.legendmod.playerSize + (PlayerCell.size) &&
          (biggercell.mass || 0) > 125 + 1.25 * ((7 - PlayerCell.mass) * 12) &&
          PlayerCell.isVirus &&
          window.legendmod.playerCells.length != 16
        ) {
          if (window.VirusFlag === true) {
            ...
            window.$("#pause-hud").html(
              "<font color='" + PlayerCell.color +
                "'>Virus</font> is close. X: " +
                parseInt(String(window.DistanceX[PlayerCell.id])) + " , Y: " +
                parseInt(String(window.DistanceY[PlayerCell.id])),
            );
          }
          ...
          distancePlayerCell <
            PlayerCell.size + window.legendmod.playerSize + 760 &&
          window.teammatenicks.includes(PlayerCell.nick) &&
          window.application.lastSentClanTag != ""
        ) {
          console.log("feed!");
          ...
          if (
            PlayerCell.mass != 0 && PlayerCell.name != "" &&
            PlayerCell.nick != null
          ) {
            doFeed = true;
          }
          window.$("#pause-hud").html(
            "<font color='" + PlayerCell.color + "'>" + PlayerCell.nick +
              "</font> (mass: " + PlayerCell.mass + ") is teammate. X: " +
              parseInt(String(window.DistanceX[PlayerCell.id])) + " , Y: " +
              parseInt(String(window.DistanceY[PlayerCell.id])),
          );
        } // Logic: Danger Cells
          ...
          (distancePlayerCell <
              PlayerCell.size + window.legendmod.playerSize + 760 &&
            PlayerCell.mass > (biggercell.mass || 0) * 2.5) ||
          (distancePlayerCell <
              PlayerCell.size + window.legendmod.playerSize + 95 &&
            PlayerCell.mass > (biggercell.mass || 0) * 1.25)
        ) {
          window.DangerDistanceX[PlayerCell.id] =
            window.DistanceX[PlayerCell.id];
          window.DangerDistanceY[PlayerCell.id] =
            window.DistanceY[PlayerCell.id];
          window.DangerDistanceName[PlayerCell.id] =
            window.DistanceName[PlayerCell.id];
          ...
          // Check Leaderboard Danger
          if (window.legendmod.playerSize > 140) {
            // Check if the dangerous cell is on the leaderboard
            const isLeaderboardDanger = window.legendmod.leaderboard.slice(0, 5)
              .some((lb) =>
                lb && lb.nick &&
                window.DangerDistanceName[PlayerCell.id] === lb.nick
              );
            ...
            if (isLeaderboardDanger) {
              for (let i = 0; i < 5 && i < window.FlagDangerCells.length; i++) {
                ...
                for (let j = 0; j < window.legendmod.leaderboard.length; j++) {
                  if (
                    window.DangerDistanceName[dId] ==
                      window.legendmod.leaderboard[j].nick
                  ) {
                    window.BadCellsDistanceX[dId] = window.DangerDistanceX[dId];
                    window.BadCellsDistanceY[dId] = window.DangerDistanceY[dId];
                    window.BadCellsDistanceName[dId] =
                      window.DangerDistanceName[dId];
                  }
                }
              }

              // UPDATED: Destructure return object
          ...
          if (
            distancePlayerCell - PlayerCell.size <= bestDist2 &&
            !VirusCellDontDoTheRest
          ) {
            if (window.BiggerCellFlag == true) {
              ...
              window.$("#pause-hud").html(
                "<font color='" + PlayerCell.color + "'>" + PlayerCell.nick +
                  "</font> (mass: " + PlayerCell.mass + ") is close. X: " +
                  parseInt(String(window.DistanceX[PlayerCell.id])) + " , Y: " +
                  parseInt(String(window.DistanceY[PlayerCell.id])),
              );
            }
            ...
            if (window.SandwichCellCase != null) {
              // Determine sandwich escape vector
        ...
        } // Logic: Hunting Smaller Cells
        else if (
          distancePlayerCell <
            PlayerCell.size + window.legendmod.playerSize + 320 &&
          PlayerCell.mass * 1.4 < (biggercell.mass || 0) &&
          (biggercell.mass || 0) > 130 &&
          !VirusCellDontDoTheRest
        ) {
          if (
            PlayerCell.mass != 0 && PlayerCell.nick != "" &&
            PlayerCell.mass * 3 < (biggercell.mass || 0) &&
            window.legendmod.playerCells.length == 1 &&
            !(PlayerCell.mass * 10 < (biggercell.mass || 0) &&
              (biggercell.mass || 0) > 260)
          ) {
            if (window.SmallerCellFlag == true) {
              ...
              window.$("#pause-hud").html(
                "<font color='" + PlayerCell.color + "'>" + PlayerCell.nick +
                  "</font> (mass: " + PlayerCell.mass +
                  ") is close and will be eaten by split. X: " +
                  parseInt(String(window.DistanceX[PlayerCell.id])) + " , Y: " +
                  parseInt(String(window.DistanceY[PlayerCell.id])),
              );
            }
  ...
  if (doSplit == true && window.doSplitFlag == true) {
    ...
    window.legendmod.sendAction(17); // Split action
    ...
    doFeed = false;
    window.legendmod.sendAction(21); // Feed action
  }
}
...
function GeneralAvoiding(target2: Point, PlayerCell: Cell): Point {
  if (window.DistanceX[PlayerCell.id] > 0) {
    target2.x = window.legendmod.mapMinX;
  } else {
    target2.x = window.legendmod.mapMaxX;
...
// Updated to return an object because JavaScript passes primitives by value
function DefineVirusshootCaseAndShoot(
  target2: Point,
  doFeed: boolean,
  VirusCellDontDoTheRest: boolean,
): { target2: Point; doFeed: boolean; stop: boolean } | null {
  if (
    window.FlagVirusCells.length > 0 &&
    Object.keys(window.BadCellsDistanceName).length > 0
  ) {
    for (let i = 0; i < window.FlagVirusCells.length; i++) {
      for (let j = 0; j < window.FlagDangerCells.length; j++) {
        ...
        if (
          Math.abs(window.VirusDistanceX[vId]) < 700 &&
          Math.abs(window.VirusDistanceX[vId]) > 20 &&
          Math.abs(window.BadCellsDistanceX[dId]) < 760
        ) {
          if (
            window.application.lastSentClanTag != "" ||
            !window.teammatenicks.includes(window.BadCellsDistanceName[dId])
          ) {
            if (
              Math.abs(window.VirusDistanceX[vId]) <
                Math.abs(window.BadCellsDistanceX[dId]) &&
              Math.abs(window.VirusDistanceY[vId]) <
                Math.abs(window.BadCellsDistanceY[dId])
            ) {
              ...
              // Logic to determine shooting direction
              let shoot = false;
...
function DefineSandwichCellCase(): void {
    ...
    for (let i = 1; i < window.FlagDangerCells.length; i++) {
      ...
      if (window.DangerDistanceX[prevId] > 0) {
        if (window.DangerDistanceY[prevId] > 0) {
          if (
            window.DangerDistanceX[currId] < 0 &&
            window.DangerDistanceY[currId] < 0
          ) {
...
function handleSandwichCellCase(target2: Point): Point {
  let closestX = 1000;
  let closestY = 1000;
  let negativeX = false;
  let negativeY = false;
...
// Updated to return object containing modified split flag
function avoidCorners(
  biggercell: Cell,
  target2: Point,
  PlayerCell: Cell,
  doSplittoAvoidCorner: boolean,
): { target2: Point; doSplit: boolean } {
  if (!biggercell || !PlayerCell) {
    return { target2, doSplit: doSplittoAvoidCorner };
  }

  if (
    (biggercell.x < window.legendmod.mapMinX + 760 ||
      biggercell.y < window.legendmod.mapMinY + 760 ||
      biggercell.x > window.legendmod.mapMaxX - 760 ||
      biggercell.y > window.legendmod.mapMaxY - 760) &&
    (PlayerCell.x < window.legendmod.mapMinX + 760 ||
      PlayerCell.y < window.legendmod.mapMinY + 760 ||
      PlayerCell.x > window.legendmod.mapMaxX - 760 ||
      PlayerCell.y > window.legendmod.mapMaxY - 760)
  ) {
    ...
    const isCloseEdge = PlayerCell.x < window.legendmod.mapMinX + 520 ||
      PlayerCell.y < window.legendmod.mapMinY + 520 ||
      PlayerCell.x > window.legendmod.mapMaxX - 520 ||
      PlayerCell.y > window.legendmod.mapMaxY - 520;
...
function calcDist(x: number, y: number): number {
  return Math.round(
    Math.sqrt(
      Math.pow(window.legendmod.playerX - x, 2) +
        Math.pow(window.legendmod.playerY - y, 2),
    ),
  );
}

// FIXED: Typo in arguments (vx, vx) -> (vx, vy)
function calcDistVirus(x: number, y: number, vx: number, vy: number): number {
  return Math.round(Math.sqrt(Math.pow(vx - x, 2) + Math.pow(vy - y, 2)));
}

function AnnounceDangerCellOpposite(name1: string, name2: string): void {
  window.$("#pause-hud").html(
    "<font color='red'>Danger: <font> " + name1 + " and " + name2 +
      " are diametral",
  );
}

function AnnounceBadCellShooting(
  vx: number,
  vy: number,
  name: string,
  x: number,
  ...
  window.$("#pause-hud").html(
    "<font color='yellow'>Shooting: <font> " + name + "(" +
      parseInt(String(x)) + " , " + parseInt(String(y)) + ")" + "from virus " +
      parseInt(String(vx)) + "," + parseInt(String(vy)),
  );
}